<div class="navigation">
    <img src = "../../../public/images/InternEaseLogo.png" alt="">
    <ul>
    <li>
            <a href="../pdc/index.php" id="dashboard">
                <span class="icon"><ion-icon name="home-outline"></ion-icon></span>
                <span class="title">Dashboard</span>
            </a>
        </li>
        <li>
            <a href="../pdc/advertisement.php" id="advertisement">
                <span class="icon"><ion-icon name="folder-open-outline"></ion-icon></span>
                <span class="title">Advertisements</span>
            </a>
        </li>
        <li>
            <a href="../pdc/managestudent.php" id="manage_student">
                <span class="icon"><ion-icon name="people-outline"></ion-icon></span>
                <span class="title">Manage Student</span>
            </a>
        </li>
        <li>
            <a href="../pdc/managecompany.php" id="manage_company">
                <span class="icon"><ion-icon name="bag-remove-outline"></ion-icon></span>
                <span class="title">Manage Company</span>
            </a>
        </li>
        <li>
            <a href="../pdc/roundselection.php" id="round_selection">
                <span class="icon"><ion-icon name="reader-outline"></ion-icon></span>
                <span class="title">Round Selection</span>
            </a>
        </li>
        <li>
            <a href="#" id="schedule">
                <span class="icon"><ion-icon name="calendar-clear-outline"></ion-icon></span>
                <span class="title">Schedule</span>
            </a>
        </li>
        <li>
            <a href="../pdc/request.php" id="request">
                <span class="icon"><ion-icon name="swap-horizontal-outline"></ion-icon></span>
                <span class="title">Request</span>
            </a>
        </li>
        <li>
            <a href="#" id="profile">
                <span class="icon"><ion-icon name="person-outline"></ion-icon></span>
                <span class="title">Profile</span>
            </a>
        </li>
        <li>
            <a href="../../controllers/signout_controller.php">
                <span class="icon"><ion-icon name="log-out-outline"></ion-icon></span>
                <span class="title">Sign Out </span>
            </a>
        </li>
    </ul>
</div>

<script src="../../../public/javascripts/sidemenu.js"></script>